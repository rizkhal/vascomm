 ...
 <ul class="space-y-2 pb-2">
  <li v-for="(navigator, index) in JSON.parse(navigators)" :key="index">
   <v-app-link
    ...
    :class="{
     'bg-gray-200': route().has(navigator.url) && currentUrl === navigator.url ? true : false,
    }"
    class="
     p-2
     flex
   ...
   >
    <v-icon
     :name="navigator.icon"
     type="outline"
     class="w-4 h-4 duration-75 text-gray-500 transition-all group-hover:text-gray-900"
    />
      ...
      <li v-for="(subNavigator, subIndex) in navigator.subItems" :key="subIndex">
       ...
       >
        <v-icon
         ...
         type="outline"
         class="w-4 h-4 duration-75 transition-all text-gray-500 group-hover:text-gray-900"
        />
